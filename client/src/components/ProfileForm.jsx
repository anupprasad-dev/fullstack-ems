        ...
        <form onSubmit={handleSubmit} className='card p-5 sm:p-6 mb-6'>
            ...
            <div className='space-y-5'>
                ...
                <div>
                    ...
                    <textarea disabled={initialData.isDeleted} name='bio' value={initialData.bio || ''} placeholder='Write a brief bio...' className={`resize-none ${initialData.isDeleted ? 'bg-slate-50 text-slate-400 cursor-not-allowed' : 'bg-white text-slate-700'}`} />
                    <p className="text-xs text-slate-500 mt-1">Your bio will be displayed on your public profile.</p>
                </div>
                {initialData.isDeleted ? (
                    <div className='pt-2'>
                        <div className='p-4 bg-rose-50 border border-rose-200 rounded-xl text-center'>
                            <p className='text-rose-600 font-medium tracking-tight'>Account Deactivated</p>
                            <p className='text-sm text-rose-50 mt-0.5'>You can no longer access your account.</p>
                        </div>
                    </div>
                ) : (
                    <div className='flex justify-end pt-2'>
                        <button className='btn-primary flex items-center gap-2 justify-center w-full sm:w-auto cursor-pointer' disabled={loading} type='submit'>
