...
import { CalendarIcon, ChevronRightIcon, FileTextIcon, LayoutGridIcon, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, XIcon } from 'lucide-react'
      ...
      <div className='px-5 pt-6 pb-5 border-b border-white/6'>
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <p className='font-semibold text-[13px] text-white tracking-wide'>Employees MS </p>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative`}>
              {isActive && <div className='absolute left-0 top-1/2 -translate-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500' />}
